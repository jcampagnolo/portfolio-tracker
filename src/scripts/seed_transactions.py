...
def fetch_all_exchange_rates(records: list[dict]) -> dict[str, float]:
    ...
    for i, d in enumerate(sorted_dates, 1):
        ...
        if rate:
            rates[d] = rate
            print(f"   [{i}/{len(sorted_dates)}] {d} → R$ {rate:.4f}")
        else:
            print(f"   [{i}/{len(sorted_dates)}] {d} → ❌ não encontrada")
...
def seed():
    ...
    # Ordem de limpeza: das mais dependentes para as menos
    tables_to_clean = [
        "transactions",
        "exchange_rates",
        "broker_accounts",
    ...
    # Verificar se alguma categoria do JSON não existe no banco
    missing_cats = set()
    for r in records:
    ...
    if missing_cats:
        ...
        for cat_name in missing_cats:
            ...
            default_curr = (
                currency_map.get("USD")
                if cat_name in ("ETF Exterior", "Stocks", "REITs")
                else currency_map.get("BRL")
            )
    ...
    if exchange_rates:
        print("💱 Inserindo cotações USD/BRL no banco...")
    ...
    for ticker, categoria in sorted(ticker_info.items()):
        ...
        currency_id = (
            currency_map["USD"]
            if categoria in CATEGORIAS_USD
            else currency_map["BRL"]
        )
    ...
    for idx, r in enumerate(records):
        ...
        if not asset_id:
            print(f"   ⚠️  Asset não encontrado: {ticker}")
            tx_errors += 1
            continue
        ...
        if not broker_id:
            print(f"   ⚠️  Broker não encontrado: {broker_name}")
            tx_errors += 1
            continue
        ...
        for entrada in entradas:
            for saida in saidas:
                ...
                if (
                    ...
                    # Vincular: entrada aponta pra saída e vice-versa
                    ...
                    linked += 1
                    print(
                        f"   🔗 {saida['ticker']} → {entrada['ticker']} "
                        f"({entrada['date']}, {entrada['broker'][:20]}...)"
                    )
                    break
    ...
    try:
        ...
        print("\n⚠️  View portfolio_positions não disponível (verificar depois)")
